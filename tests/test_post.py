...
@pytest.mark.asyncio
async def test_get_post_not_exists(clean_client):
    response = await clean_client.get("/posts/999")
...
@pytest.mark.asyncio
async def test_get_post_success(clean_client, registered_post):
    response = await clean_client.get(f"/posts/{registered_post}")
    assert response.status_code == 200
